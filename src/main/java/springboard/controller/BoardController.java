...
public class BoardController {
	...
	public String list(Model model, HttpServletRequest req) {
		...
		/*
		기본 버전에서는 해당 서비스 객체를 new를 통해 생성했지만,
	...
	/*
	글쓰기 처리에 대한 매핑처리를 한다. 전송방식은 post로 전송시 요청명은
	value 속성에 기술한다.
	 */
	...
	public String writeAction(Model model, HttpServletRequest req,
		...
		따라서 폼값을 받기위해 getParamter()메서드를 일일히 사용할
		필요가 없어진다.
		 */
	...
	public String view(Model model, HttpServletRequest req) {
		/*
		목록에서 제목을 클릭할때 전달되는 파라미터는 idx, pageNum등을
	...
	public String password(Model model, HttpServletRequest req) {
		...
		받은 후 모델객체에 저장한다. 나머지 값은 View에서 직접 EL을
		통해 받을것이다.
		 */
		model.addAttribute("idx", req.getParameter("idx"));
		return "BoardSkin/passwordT";
	}
	...
	public String passwordAction(Model model, HttpServletRequest req) {
		...
		if(existIdx<=0) {
			...
			if(mode.equals("edit")) {
				//수정인 경우 서비스객체러 모델객체를 전달하고..
				model.addAttribute("req", req);
				service = this.editExecute;
				service.execute(model);
				//수정페이지를 포워드한다.
				modePage = "BoardSkin/editT";
			}
